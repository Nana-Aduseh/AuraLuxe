  ...
  const enrichedOrders = await Promise.all(
    (orders ?? []).map(async (order: any) => {
      const [{ data: items }, { data: profile }] = await Promise.all([
        ...
        dataClient.from('profiles').select('id, name, email').eq('id', order.user_id).maybeSingle(),
      ])

      const orderItems = await Promise.all(
        (items ?? []).map(async (item: any) => {
          const [{ data: product }, { data: color }, { data: quantity }] = await Promise.all([
            dataClient.from('products').select('name').eq('id', item.product_id).maybeSingle(),
            item.color_id
              ? dataClient.from('product_colors').select('color_name').eq('id', item.color_id).maybeSingle()
              : Promise.resolve({ data: null }),
            item.quantity_id
              ? dataClient.from('product_quantities').select('length_inches').eq('id', item.quantity_id).maybeSingle()
              : Promise.resolve({ data: null }),
          ])
